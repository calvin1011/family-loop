...
interface CommunicationGoal {
  ...
  contactId: string;
  contactName: string;
  frequency: 'daily' | 'every-2-days' | 'every-3-days' | 'weekly' | 'bi-weekly' | 'monthly';
  frequencyDays: number;
  method: 'call' | 'text' | 'any';
  isActive: boolean;
  lastContacted?: Date;
  nextDue?: Date;
  customNote?: string;
...
const METHOD_OPTIONS = [
  ...
  { value: 'call', label: 'Phone calls only', icon: '📞' },
  { value: 'text', label: 'Messages only', icon: '💬' },
];

export function CommunicationGoals({ contacts, interactions, onGoalCreated }: Props) {
  const [goals, setGoals] = useState<CommunicationGoal[]>([]);
    ...
    try {
      ...
      if (stored) {
        const parsedGoals = JSON.parse(stored).map((goal: any) => ({
          ...goal,
          lastContacted: goal.lastContacted ? new Date(goal.lastContacted) : undefined,
          nextDue: goal.nextDue ? new Date(goal.nextDue) : undefined,
        }));
  ...
  const getGoalStatus = (goal: CommunicationGoal) => {
    if (!goal.nextDue || !goal.isActive) return { status: 'inactive', color: '#95a5a6', text: 'Inactive' };

    const now = new Date();
    const timeDiff = goal.nextDue.getTime() - now.getTime();
  ...
  const getDueGoalsCount = () => {
    ...
    return goals.filter(goal => {
      if (!goal.isActive || !goal.nextDue) return false;
      return goal.nextDue.getTime() <= now.getTime();
    }).length;
  };
      ...
      {dueCount > 0 && (
        <View style={styles.dueGoalsSection}>
          ...
          {goals
            .filter(goal => {
              if (!goal.isActive || !goal.nextDue) return false;
              return goal.nextDue.getTime() <= new Date().getTime();
            })
            .slice(0, 3)
            .map(goal => (
